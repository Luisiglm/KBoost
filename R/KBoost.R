...
# TFs a Kx1 numeric matrix with integers that are the indexes of columns of X with genes that encode TFs.
...
# prior_weights a GxK matrix with the prior weights for the prior probability that a TF regulates a gene.(default 0.5 for all values)
...
# lliks a GxK numeric matrix with the log-marginal likelihoods of TF regulation models.
...
#
# posterior_list a list with GxK numeric matrix with the log-posteriors of TF regulation models.
# model_list a list with GxK logical matrix that indicates if a TF was part of a regulation model.
...
# returns: a list with b: the regression coefficients and llik the marginal loglik.
#
...
################################################################################
#
# A Function called prior model calculate the prior given a set of prior weights and a model.
# prior_weights a matrix with the prior probability to include a Tf in a model.
# model a logical matrix whose elements are TRUE if a TF is in the model of a gene.
